...
export default class AddRecordForm extends Component {
    ...
    render() {
        return (
            ...
            <View flexDirection='row' justifyContent='space-around'>
              <TextInput placeholder='ФИО...' onChangeText={(text) => this.props.onChange(text, 'name')}/>
              <TextInput 
              placeholder='Сумма...' 
              onChangeText={text => this.onChangeSum(text)}
              />
              <TextInput placeholder='Примечание...' onChangeText={(text) => this.props.onChange(text, 'text')}/>
            </View>
            <View flexDirection='column' justifyContent='space-around'>
            <Button
            onPress={() => this.props.addRecord('save')}
            title="Сохранить"
            />
            <Button
            onPress={() => this.props.addRecord(null)}
            title="Отмена"
            />
            </View>
        </View>
